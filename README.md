...
- Host OS: Windows
- User Context: Initial setup performed as root, subsequent tasks executed with a sudo-enabled user<br>
...
This section describes the steps required to enable remote administration of the Fedora system.<br>
...
After sudo privileges were granted to user1, the system was logged into using the user1 account. The hostname was changed using sudo privileges, and direct login to the root account was disabled.<br>
...
<img width="980" height="630" alt="image" src="https://github.com/user-attachments/assets/6670d4a2-c584-42e8-9879-7adb485bfb19" /><br><br>
As shown in the figures above, user3 is able to fully exercise its assigned permissions on the shared directory. Specifically:<br>
user3 can access, read, and edit files that were previously added to the share.<br>
user3 can edit files and folders created by both user1 and user2.<br>
user3 can create and modify its own files and folders within the shared directory.<br>
These observations confirm that user3 has complete access and modification rights, demonstrating that the permission settings on the shared resource are correctly applied.
<br><br>
<img width="1111" height="626" alt="image" src="https://github.com/user-attachments/assets/343c0b1e-1e34-4052-8316-5d024942381f" />
<pre><code>taskkill /f /im explorer.exe
#taskkill /f /im explorer.exe resets the File Explorer session and prevents UI‑level SMB credential reuse. It does not terminate SMB sessions on its own.This command prevents Windows from automatically reconnecting to the SMB share using user3’s credentials after the active SMB session has been terminated.
start explorer.exe
#This command is used to start (or restart) the Windows Explorer process, which provides the main graphical user interface of Windows.
net use /delete *
#net use /delete * disconnects active smb connections. (user3 disconnects)
net use \\192.168.1.28\Share1 /user:user4 *
#This command is used to establish an SMB network connection to the shared folder using the user4 account.</pre></code>
...
<img width="833" height="783" alt="image" src="https://github.com/user-attachments/assets/e2605f87-959c-45a8-9c2d-c8cdf63fb591" <br><br>
...
From this point onward, the system will no longer prompt for credentials for the second shared folder, and we will be able to connect to it as the Guest user.
<br><br>
